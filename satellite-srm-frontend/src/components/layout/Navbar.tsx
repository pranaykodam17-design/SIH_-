    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled || menuOpen ? 'navbar-blur shadow-[0_1px_30px_rgba(0,0,0,0.4)]' : 'bg-transparent'
        }`}
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between gap-4">
          ...
          <Link to="/" className="flex items-center gap-3 flex-shrink-0 group">
            ...
            <div className="hidden sm:block">
              <div className="text-base font-black text-white tracking-tight leading-none">SRM</div>
              <div className="text-[10px] font-medium text-slate-500 tracking-wider leading-none mt-0.5">
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <button
              className="w-9 h-9 flex items-center justify-center rounded-xl text-slate-500 hover:text-slate-300 hover:bg-white/5 transition-all duration-200 relative"
              title="Notifications"
            >
              <Bell size={16} />
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className="lg:hidden navbar-blur border-t border-white/[0.06] px-4 py-3 space-y-1 animate-fade-in">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.to}
                to={link.to}
