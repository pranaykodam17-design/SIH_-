...
const Validation2DFallback: React.FC<{
  ...
  <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center select-none bg-gradient-to-b from-srm-surface to-srm-elevated">
    <div className="w-20 h-20 rounded-full border-2 border-dashed border-cyan-200 flex items-center justify-center animate-spin-slow mb-4">
      {phase === 'scanning' && <Loader2 size={32} className="text-[#1677FF] animate-spin" />}
...
export const ValidationScannerModal: React.FC<ValidationScannerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-white backdrop-blur-md anim-fade-in select-none">
      <div className="relative w-full max-w-4xl rounded-2xl border border-[#D7E6F4] bg-white/95 shadow-[0_0_60px_rgba(0,212,255,0.15)] overflow-hidden flex flex-col">
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm sm:text-base font-bold text-[#10233F] tracking-wide flex items-center gap-2">
                ...
                {isScanning && (
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-[#1677FF]/20 border border-cyan-200 text-cyan-700 animate-pulse">
                    SCANNING
                  </span>
                )}
                {isSuccess && (
        ...
        <div className="grid md:grid-cols-2 gap-0">
          ...
          <div className="relative h-[260px] md:h-[340px] bg-white border-b md:border-b-0 md:border-r border-[#D7E6F4] overflow-hidden">
            {hasWebGL ? (
            ...
            <div className="absolute top-3 left-3 pointer-events-none z-10 flex items-center gap-1.5 px-2.5 py-1 rounded bg-white border border-[#D7E6F4] text-[10px] font-mono text-cyan-700">
              <Globe size={11} className="text-[#1677FF]" />
              <span>Footprint: UTM 44N (78.45°E, 17.41°N)</span>
            </div>

            {/* Scan mode watermark */}
            <div className="absolute bottom-3 left-3 pointer-events-none z-10 text-[10px] font-mono text-[#6B7F95]">
              Sensor: Sentinel-2 MSI · Band Calibration
            </div>
          </div>

          {/* Right: Validation Checks List */}
          <div className="p-5 md:p-6 space-y-4 flex flex-col justify-between bg-white/60">
            <div>
              ...
              <div className="space-y-2">
                {/* 1. Band Check: B02 */}
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-white border border-[#D7E6F4] text-xs">
                  <div className="flex items-center gap-2.5">
                    {isSuccess && b02 ? (
                      ...
                      <Loader2 size={14} className="text-[#1677FF] animate-spin shrink-0" />
                    )}
                    <span className="font-semibold text-[#425873]">B02 Blue (490 nm)</span>
                  </div>
                  <span className="font-mono text-[11px] text-[#526A82]">
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-white border border-[#D7E6F4] text-xs">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-semibold text-[#425873]">B03 Green (560 nm)</span>
                  </div>
                  <span className="font-mono text-[11px] text-[#526A82]">
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-white border border-[#D7E6F4] text-xs">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-semibold text-[#425873]">B04 Red (665 nm)</span>
                  </div>
                  <span className="font-mono text-[11px] text-[#526A82]">
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-white border border-[#D7E6F4] text-xs">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-semibold text-[#425873]">B08 NIR (842 nm)</span>
                  </div>
                  <span className="font-mono text-[11px] text-[#526A82]">
                ...
                <div className="pt-2 grid grid-cols-2 gap-2 text-[11px] font-mono">
                  <div className="p-2 rounded-lg bg-white border border-[#D7E6F4]">
                    <div className="text-[10px] text-[#6B7F95]">Dimensions</div>
                    <div className="text-[#425873] font-semibold flex items-center gap-1 mt-0.5">
                      {isSuccess ? (
                        <>
                        ...
                        <span className="text-[#526A82]">Checking…</span>
                      )}
                    </div>
                  </div>

                  <div className="p-2 rounded-lg bg-white border border-[#D7E6F4]">
                    <div className="text-[10px] text-[#6B7F95]">Resolution</div>
                    <div className="text-[#425873] font-semibold flex items-center gap-1 mt-0.5">
                      {isSuccess ? (
                        <>
                          <CheckCircle2 size={11} className="text-emerald-600" />
                        ...
                        <span className="text-[#526A82]">N/A</span>
                      ) : (
                        <span className="text-[#526A82]">Checking…</span>
                      )}
                    </div>
                  ...
                  <div className="p-2 rounded-lg bg-white border border-[#D7E6F4]">
                    <div className="text-[10px] text-[#6B7F95]">CRS</div>
                    <div className="text-[#425873] font-semibold flex items-center gap-1 mt-0.5 truncate">
                      {isSuccess ? (
                        <>
                        ...
                        <span className="text-[#526A82]">Parsing tags…</span>
                      )}
                    </div>
                  </div>

                  <div className="p-2 rounded-lg bg-white border border-[#D7E6F4]">
                    <div className="text-[10px] text-[#6B7F95]">Spatial Alignment</div>
                    <div className="text-[#425873] font-semibold flex items-center gap-1 mt-0.5 truncate">
                      {isSuccess ? (
                        <>
                          <CheckCircle2 size={11} className="text-emerald-600 shrink-0" />
            ...
            <div className="pt-3 border-t border-[#D7E6F4]">
              {isScanning && (
                <div className="flex items-center gap-2.5 text-xs text-cyan-700 font-mono">
                  <Loader2 size={14} className="animate-spin text-[#1677FF] shrink-0" />
                ...
                <div className="space-y-3 anim-fade-in">
                  ...
                  <div className="flex justify-end">
                    <button
                      type="button"
                      onClick={onClose}
                      className="px-4 py-2 rounded-xl bg-white hover:bg-white border border-[#D7E6F4] text-[#425873] text-xs font-semibold transition-all"
                    >
