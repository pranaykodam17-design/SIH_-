  ...
  return (
    <div
      className={`space-y-6 rounded-2xl border transition-all duration-300 p-6 ${
        isDragOverAll
          ...
          : 'border-white/[0.08] bg-[#071322]/80'
      }`}
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/[0.08] pb-5">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-400/30 text-cyan-300 font-mono text-xs font-semibold uppercase tracking-wider mb-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={disabled}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-slate-300 text-xs font-semibold transition-all"
          >
            <FileUp size={14} />
            ...
            <div
              ...
              className={`relative rounded-xl border p-4 transition-all duration-200 ${
                isSlotActive
                  ...
                  : `border-white/[0.08] bg-[#08182b]/60 ${config.borderColor}`
              }`}
            >
              {/* Slot Header */}
              ...
              {file ? (
                <div className="flex items-center justify-between gap-3 p-3 rounded-lg bg-black/40 border border-white/[0.08]">
                  <div className="flex items-center gap-2.5 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-xs font-mono font-medium text-white truncate" title={file.name}>
                        {file.name}
                      </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <div className="relative group">
                      <select
                        ...
                        className="text-[10px] font-mono bg-white/[0.06] hover:bg-white/[0.1] border border-white/10 rounded px-2 py-1 text-slate-300 focus:outline-none focus:border-cyan-400 cursor-pointer"
                        title="Change assigned band"
                      >
                        <option value="b02" className="bg-slate-900">B02 Blue</option>
                        <option value="b03" className="bg-slate-900">B03 Green</option>
                        <option value="b04" className="bg-slate-900">B04 Red</option>
                        <option value="b08" className="bg-slate-900">B08 NIR</option>
                      </select>
                    </div>
                ...
                <div
                  ...
                  className="flex flex-col items-center justify-center p-5 rounded-lg border border-dashed border-white/10 hover:border-cyan-400/50 bg-white/[0.01] hover:bg-cyan-500/[0.04] cursor-pointer transition-all text-center group"
                >
                  <Upload size={18} className="text-slate-500 group-hover:text-cyan-400 transition-colors mb-1.5" />
                  <span className="text-xs font-semibold text-slate-300 group-hover:text-cyan-300 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-white/[0.08]">
        <div className="flex items-center gap-2 text-xs font-mono">
          <span className="text-slate-400">Bands Loaded:</span>
          <span className={`font-bold ${loadedCount === 4 ? 'text-emerald-400' : 'text-amber-400'}`}>
            {loadedCount} / 4
          </span>
          <span className="text-slate-600">|</span>
          <span className="text-slate-400">Order:</span>
          <span className="text-slate-300">B02 → B03 → B04 → B08</span>
        </div>
          ...
          <button
            ...
            className={`inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300 shadow-lg ${
              allBandsLoaded && !isValidating && !disabled
                ? 'bg-gradient-to-r from-cyan-400 via-blue-500 to-cyan-400 text-slate-950 shadow-[0_0_25px_rgba(0,212,255,0.4)] hover:shadow-[0_0_35px_rgba(0,212,255,0.6)] cursor-pointer'
