...
export const BandMetadataPanel: React.FC<BandMetadataPanelProps> = ({
  ...
  const bandList = [
    { key: 'b02', label: 'Band 1: B02', title: 'Blue (490 nm)', color: 'text-blue-400', border: 'border-blue-500/30', bg: 'bg-blue-500/10' },
    { key: 'b03', label: 'Band 2: B03', title: 'Green (560 nm)', color: 'text-emerald-400', border: 'border-emerald-500/30', bg: 'bg-emerald-500/10' },
    { key: 'b04', label: 'Band 3: B04', title: 'Red (665 nm)', color: 'text-red-400', border: 'border-red-500/30', bg: 'bg-red-500/10' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-5 border border-emerald-500/30 bg-emerald-500/[0.05] flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono text-cyan-400 bg-cyan-500/10 border border-cyan-500/25 px-3 py-1.5 rounded-lg flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {bandList.map(({ key, label, title, color, border, bg }) => {
          ...
          return (
            <div
              key={key}
              className={`glass rounded-xl p-4 border ${border} bg-[#061424]/70 space-y-3 flex flex-col justify-between`}
            >
              <div>
                {/* Header */}
                ...
                <div className="space-y-2 text-xs font-mono">
                  <div>
                    ...
                    <span className="text-slate-200 text-[11px] truncate block font-medium" title={bMeta.filename}>
                      {bMeta.filename}
                    </span>
                  </div>
                  ...
                  <div className="pt-1">
                    <span className="text-[10px] text-slate-500 block">CRS / Projection</span>
                    <span className="text-slate-300 text-[11px] truncate block" title={bMeta.crs}>
                      {bMeta.crs}
                    </span>
                  </div>
      ...
      {common && (
        <div className="glass rounded-xl p-5 border border-white/[0.08] space-y-3 bg-[#061424]/40">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
            <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06]">
              <span className="text-[10px] text-slate-500 block">West (Min Lon)</span>
              <span className="text-slate-200 font-semibold">{common.bounds?.minLon}° E</span>
            </div>
            <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06]">
              <span className="text-[10px] text-slate-500 block">East (Max Lon)</span>
              <span className="text-slate-200 font-semibold">{common.bounds?.maxLon}° E</span>
            </div>
            <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06]">
              <span className="text-[10px] text-slate-500 block">South (Min Lat)</span>
              <span className="text-slate-200 font-semibold">{common.bounds?.minLat}° N</span>
            </div>
            <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.06]">
              <span className="text-[10px] text-slate-500 block">North (Max Lat)</span>
              <span className="text-slate-200 font-semibold">{common.bounds?.maxLat}° N</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-4 pt-4 border-t border-white/[0.08]">
        <button
          ...
          onClick={onBack}
          disabled={isRunning}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-slate-300 hover:text-white text-xs font-semibold transition-all"
        >
          <ArrowLeft size={14} />
        ...
        <button
          ...
          disabled={isRunning}
          className="inline-flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-cyan-400 via-blue-500 to-cyan-400 text-slate-950 font-bold text-sm shadow-[0_0_25px_rgba(0,212,255,0.4)] hover:shadow-[0_0_35px_rgba(0,212,255,0.7)] transition-all cursor-pointer disabled:opacity-50"
        >
          <Zap size={16} />
