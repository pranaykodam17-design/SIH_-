...
export const NDVIViewer: React.FC<NDVIViewerProps> = ({ ndviUrl }) => {
  return (
    <Card className="border-blue-100 bg-white p-4 sm:p-6 shadow-sm">
      ...
      <div className="relative h-80 sm:h-[450px] w-full rounded-xl border border-blue-100 overflow-hidden bg-[#F5FAFF] mb-6">
        <img
          src={ndviUrl}
          alt="NDVI comparison raster"
          className="h-full w-full object-contain bg-[#F5FAFF]"
        />
        <div className="absolute top-3 left-3 z-10 rounded bg-white/85 px-3 py-1.5 text-xs font-mono text-emerald-700 border border-emerald-500/40 backdrop-blur-md">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs mb-4">
        ...
        <div className="p-3 rounded bg-white border border-blue-100">
          <span className="text-[#6B7F95] text-[10px] uppercase block mb-1">Pearson Correlation</span>
          <span className="font-bold text-emerald-600 text-sm">
            r = {DEMO_METRICS.ndvi_correlation.model.toFixed(4)}
          </span>
          <span className="text-[10px] text-[#526A82] block mt-0.5">vs bicubic 0.9987</span>
        </div>

        <div className="p-3 rounded bg-white border border-blue-100">
          <span className="text-[#6B7F95] text-[10px] uppercase block mb-1">Mean Absolute Error</span>
          <span className="font-bold text-cyan-700 text-sm">
            {DEMO_METRICS.ndvi_mae.model.toFixed(4)}
          </span>
          <span className="text-[10px] text-[#526A82] block mt-0.5">MAE across scene</span>
        </div>

        <div className="p-3 rounded bg-white border border-blue-100">
          <span className="text-[#6B7F95] text-[10px] uppercase block mb-1">Spectral Formulation</span>
          <span className="font-bold text-[#10233F] text-sm">
            (B08 - B04) / (B08 + B04)
          </span>
          <span className="text-[10px] text-[#526A82] block mt-0.5">NIR and Red Channels</span>
        </div>

        <div className="p-3 rounded bg-white border border-blue-100">
          <span className="text-[#6B7F95] text-[10px] uppercase block mb-1">Vegetation Fidelity</span>
          <span className="font-bold text-emerald-700 text-sm">
            Preserved
          </span>
          <span className="text-[10px] text-[#526A82] block mt-0.5">L_NDVI loss enforced</span>
        </div>
      ...
      <div className="pt-4 border-t border-blue-100 font-mono text-xs">
        <span className="text-[#526A82] block mb-1">NDVI Reflectance Scale:</span>
